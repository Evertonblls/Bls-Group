...
app.post('/api/clients', (req, res) => {
  const { nome, origem = 'Indicação', mrr = 0, entrada = 'mar/26' } = req.body;
...
app.post('/api/tasks', (req, res) => {
  const { text, cliente = '—', prioridade = 'Média', status = 'Pendente', prazo = '—' } = req.body;
...
app.post('/api/command', (req, res) => {
  ...
  try {
    switch (action) {
      ...
      case 'add_task': {
        const { text, cliente = '—', prioridade = 'Média', prazo = '—' } = data;
      ...
      case 'add_client': {
        const { nome, mrr, origem = 'Indicação' } = data;
